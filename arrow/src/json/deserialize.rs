...
trait Powi10: NativeType + num::One + std::ops::Add {
...
trait Pow10: NativeType + num::One + std::ops::Add {
...
>(
    ...
    unsafe { PrimitiveArray::from_trusted_len_iter(iter).to(data_type) }
}
...
fn deserialize_binary<'a, O: OffsetSizeTrait, A: Borrow<Value<'a>>>(
    rows: &[A],
    ...
    unsafe { BinaryArray::from_trusted_len_iter(iter) }
}

fn deserialize_utf8<'a, OffsetSize: OffsetSizeTrait, A: Borrow<Value<'a>>>(
    rows: &[A],
) -> GenericStringArray<OffsetSize> {
    let mut array =
        GenericStringBuilder::<OffsetSize>::with_capacity(rows.len(), rows.len() * 5);
    let mut scratch = vec![];
    for row in rows {
        match row.borrow() {
...
fn deserialize_list<'a, O: OffsetSizeTrait, A: Borrow<Value<'a>>>(
    rows: &[A],
    data_type: DataType,
) -> ListArray<O> {
    ...
    rows.iter().fold(O::zero(), |mut length, row| {
        match row.borrow() {
            Value::Array(value) => {
                ...
                length +=
                    O::from_usize(value.len()).expect("List offset is too large :/");
                offsets.push(length);
                length
            }
            _ => {
...
fn deserialize_struct<'a, A: Borrow<Value<'a>>>(
    rows: &[A],
    data_type: DataType,
) -> StructArray {
            ...
            Some((hash, v)) => match map.get(&hash) {
                ...
                None => {
                    let key = match map.len().try_into() {
                        ...
                        Err(_) => {
                            panic!("The maximum key is too small for this json struct")
                        }
                    };
                    inner.push(v);
                    map.insert(hash, key);
...
pub(crate) fn _deserialize<'a, A: Borrow<Value<'a>>>(
    ...
    match &data_type {
        DataType::Null => Box::new(NullArray::new(rows.len())),
...
pub fn deserialize(json: &Value, data_type: DataType) -> Result<Box<dyn Array>> {
    match json {
        Value::Array(rows) => match data_type {
            ...
            _ => Err(ArrowError::ParseError(
                "read an Array from a non-Array data type".to_string(),
            )),
        },
        _ => Err(ArrowError::ParseError(
            "read an Array from a non-Array JSON".to_string(),
        )),
    }
}*/
